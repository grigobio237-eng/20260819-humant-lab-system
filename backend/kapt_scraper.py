...
async def scrape_kapt_bids(limit: int = 10):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # 리스트 테이블 파싱 (K-apt의 경우 onclick 팝업 또는 링크로 상세페이지 이동)
            ...
            for row in rows[:limit]:
                try:
                    ...
                    # 3. K-apt 첨부파일 다운로드 로직 및 AI 파서 연동
                    attachment_dir = f"storage/attachments/{bid_no}"
                    os.makedirs(attachment_dir, exist_ok=True)
                    
                    # K-apt의 첨부파일 링크 다운로드 (주로 a 태그 중 다운로드 아이콘 등)
                    ...
                    # 상세페이지 전체 텍스트를 파서에 넘겨 A값/하한율 추출 (민간공사는 본문에 적혀있는 경우도 많음)
