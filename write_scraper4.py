...
async def scrape_kapt_bids(limit: int = 10):
    print("DEBUG: scrape_kapt_bids STARTED!")
    sys.stdout.flush()
    ...
    async with async_playwright() as p:
        ...
        try:
            print("DEBUG: Going to K-apt url...")
            sys.stdout.flush()
            await page.goto(url, wait_until="domcontentloaded", timeout=60000)
            print("DEBUG: URL loaded, waiting for selector...")
            sys.stdout.flush()
            ...
            for i, row in enumerate(rows):
                ...
                try:
                    ...
                    if not match:
                        print(f"DEBUG: Row {i} no goView match. HTML: {row_html[:100]}")
                        sys.stdout.flush()
                        continue
                        
                    bid_num = match.group(1)
                    print(f"DEBUG: Row {i} processing bid {bid_num}...")
                    sys.stdout.flush()
                    
                    async with context.expect_page() as new_page_info:
                        await row.evaluate("(element) => element.querySelector('td[onclick]').click()")
                    
                    detail_page = await new_page_info.value
                    print(f"DEBUG: Row {i} popup opened, waiting for load...")
                    sys.stdout.flush()
                    ...
                    if download_links:
                        try:
                            ...
                            print(f"DEBUG: K-apt download error: {e}")
                            sys.stdout.flush()
                    ...
                    print(f"DEBUG: Row {i} successfully appended! bid_no: {bid_no}")
                    sys.stdout.flush()
                    
                    await detail_page.close()
                except Exception as e:
                    print(f"DEBUG: K-apt row parsing error: {e}")
                    sys.stdout.flush()
                    
        except Exception as e:
            print(f"DEBUG: K-apt Scraper Error: {e}")
            sys.stdout.flush()
        finally:
            await browser.close()
