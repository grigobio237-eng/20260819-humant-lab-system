        ...
        try:
            ...
            for row in rows:
                ...
                try:
                    ...
                    # GET 방식으로는 내용이 안 나옴. Javascript goView() 실행 필요!
                    detail_url = "https://www.k-apt.go.kr/bid/bidDetail.do"
                    
                    # POST 방식으로 데이터 가져오기 위해 evaluate 사용 (또는 goto 후 스크립트 실행)
                    # 리스트 페이지(현재 page)에서 팝업/새 탭을 띄우게 하면 가장 안전함
                    ...
                    if download_links:
                        try:
                            ...
                            print(f"K-apt download error: {e}")
                            sys.stdout.flush()
